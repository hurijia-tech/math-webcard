...
# ── 기존 cards_data.json에서 question_type 보존 ──
# 이미 수동으로 변경한 형태 데이터를 덮어쓰지 않기 위해
existing_types = {}
...
if out_path.exists():
    try:
        ...
        for card in existing:
            if card.get("question_type") and card["question_type"] != "주관식":
                existing_types[card["id"]] = card["question_type"]
        print(f"ℹ️  기존 형태 데이터 보존: {len(existing_types)}개")
    except:
        pass
    ...
    with open(csv_path, encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            card = {
                ...
                "question_type":    existing_types.get(card_id, "주관식"),  # ← 기본값: 주관식, 기존 변경값 보존
